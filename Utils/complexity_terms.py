...
def get_hyper_divergnce(kappa_prior,
                        kappa_post,
                        divergence_type,
                        device,
                        prior_model):   # corrected
    # 计算超级先验与后验的散度divergence
    ...
    elif divergence_type == 'W_NoSqr':
        d = prior_model.weights_count
        hyper_dvrg = torch.sqrt(net_weights_magnitude(prior_model, device, p=2) + d * (kappa_prior - kappa_post) ** 2)

    elif divergence_type == 'W_Sqr':
        d = prior_model.weights_count
        hyper_dvrg = net_weights_magnitude(prior_model, device, p=2) + d * (kappa_prior - kappa_post) ** 2

    else:
        raise ValueError('Invalid divergence_type')
...
def get_meta_complexity_term(hyper_kl,
                             delta,
                             complexity_type,
                             n_train_tasks):   # corrected
    if n_train_tasks == 0:
        meta_complex_term = 0.0  # infinite tasks case
    else:
        if complexity_type in {'McAllester', 'Seeger'}:
            delta = delta
...
# -----------------------------------------------------------------------------------------------------------#
# 计算不同任务间的 complexity item
# 根据不同的PAC-Bayes Bound Theory，选择不同的complexity item
def get_task_complexity(delta,
                        complexity_type,
                        device,
                        divergence_type,
                        kappa_post,
    ...
    if not dvrg:
        ...
        dvrg = get_net_densities_divergence(prior_model,
                                            post_model,
                                            kappa_post,
                                            divergence_type,
                                            noised_prior)
    ...
    if complexity_type == 'NoComplexity':
        # set as zero
        complex_term = torch.zeros(1, requires_grad=False, device=device)

    elif complexity_type == 'McAllester':
...
# 计算不同分布的距离
def get_net_densities_divergence(prior_model,
                                 post_model,
                                 kappa_post,
                                 divergence_type,
                                 noised_prior=False):
    ...
    for i_layer, prior_layer in enumerate(prior_layers_list):
        ...
        if hasattr(prior_layer, 'w'):
            total_dvrg += get_dvrg_element(post_layer.w,
                                           prior_layer.w,
                                           kappa_post,
                                           divergence_type,
                                           noised_prior)
        if hasattr(prior_layer, 'b'):
            total_dvrg += get_dvrg_element(post_layer.b,
                                           prior_layer.b,
                                           kappa_post,
                                           divergence_type,
                                           noised_prior)

    if divergence_type == 'W_NoSqr':
        total_dvrg = torch.sqrt(total_dvrg)

    return total_dvrg
# -------------------------------------------------------------------------------------------

def  get_dvrg_element(post,
                      prior,
                      kappa_post,
                      divergence_type,
                      noised_prior=False):
    ...
    if  divergence_type in ['W_Sqr', 'W_NoSqr']:
